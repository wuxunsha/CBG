<template>
    <div>
        ...
        <div>
            <van-nav-bar :title="$t('wallet.share.nav_title')"
                         left-arrow
                         @click-left="goback()" />
        </div>
        ...
        <div class="invitation-code">
            <h2 v-if="area=='left'">{{userInfo.userInvitation}}</h2>
            <h2 v-else>{{userInfo.userInvitationRight}}</h2>
        </div>

        <!-- 复制按钮 -->
        <div class="copycode"
             :data-clipboard-text="code_text">
            <span @click="copy">
